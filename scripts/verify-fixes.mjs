...
assert(
  setupMd.includes('Node 22 is pinned in `.nvmrc`') && setupMd.includes('nvm use'),
  'setup.md directs developers to use the Node 22 version pinned in .nvmrc'
);
assert(
...
assert(buildingMd.includes('on Node 22'), 'building.md states CI runs on Node 22');
assert(
